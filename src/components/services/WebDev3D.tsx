      ...
      <Float speed={1.2} rotationIntensity={0.3} floatIntensity={0.6}>
        <group ref={group}>
          ...
          <RoundedBox args={[4, 2.6, 0.15]} radius={0.12} smoothness={4}>
            <meshStandardMaterial color="#0d1530" metalness={0.6} roughness={0.3} />
          </RoundedBox>
          {/* top bar */}
          <mesh position={[0, 1.05, 0.08]}>
            <planeGeometry args={[4, 0.5]} />
            <meshBasicMaterial color="#1a2350" />
          </mesh>
          {/* dots */}
          {[-1.7, -1.5, -1.3].map((x, i) => (
          ...
          {codeLines.slice(0, step).map((line, i) => (
            <Text key={i} position={[-1.7, 0.55 - i * 0.32, 0.1]} fontSize={0.18} color="#8aa6ff" anchorX="left" anchorY="middle">
              {line}
            </Text>
          ))}
          {/* cursor */}
          <mesh position={[-1.7 + step * 0.05, 0.55 - Math.min(step, codeLines.length - 1) * 0.32, 0.1]}>
            ...
            <meshBasicMaterial color="#3565F5" />
          </mesh>
        </group>
      </Float>
    </>
