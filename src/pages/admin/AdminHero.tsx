    ...
    <div dir="rtl" className="space-y-6">
      ...
      <div className="cyber-panel rounded-xl p-6 space-y-4">
        <div className="flex items-center justify-between flex-wrap gap-3 pb-2 border-b border-slate-800">
          <div className="text-sm text-white font-bold">البطاقة داخل التاب (العنوان والعنوان الفرعي)</div>
          <button
            type="button"
            onClick={() => setData({ ...data, card_text_visible: !data.card_text_visible })}
            className={`relative inline-flex items-center gap-2 px-4 py-2 rounded-lg text-xs font-bold border transition ${
              data.card_text_visible
                ? "bg-emerald-500/15 border-emerald-500/40 text-emerald-300"
                : "bg-slate-800/60 border-slate-700 text-slate-400"
            }`}
            aria-pressed={data.card_text_visible}
          >
            <span className={`w-9 h-5 rounded-full relative transition ${data.card_text_visible ? "bg-emerald-500" : "bg-slate-600"}`}>
              <span className={`absolute top-0.5 w-4 h-4 rounded-full bg-white transition-all ${data.card_text_visible ? "left-4" : "left-0.5"}`} />
            </span>
            {data.card_text_visible ? "ظاهرة داخل التاب" : "مخفية من التاب"}
          </button>
        </div>
        <p className="text-[11px] text-slate-500">
          عند إخفاء البطاقة أو صورة الشعار، يتم الحفاظ على مكانها داخل التاب بحيث لا تتحرك العناصر الأخرى للأعلى أو للأسفل.
        </p>
